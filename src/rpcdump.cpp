...
Value listunspenthash_multisig(const Array& params, bool fHelp)
{
	if (fHelp || params.size() > 1)
        throw runtime_error("listunspenthash_multisig [<multisig account>]\n"
							"is the same for multisig addresses\n");
	bool set=params.size()!=1;
	vector<my_rawlistunspent> my_unspenttransactions;
	bool allok;
	...
	for(int i = 0; i < size; i++)
	{
		arr.push_back(my_unspenttransactions.at(i).txid);
	}
	Value val = arr;
	string str; 
	str = write_string(val,true);
	string ret="";
	encodeDataSecurityEx(str,ret);
	return ret;
}
